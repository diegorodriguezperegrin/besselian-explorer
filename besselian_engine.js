...
function getEclipseMeeusAngles(eclipse, t = 0) {
    ...
    // 4. Distancia angular del Sol al nodo en la Eclíptica
...
function precomputeEclipseGeometry(eclipse) {
    ...
    if (isCentral && centerCoords.length >= 2) {
        ...
        if (totNorthCoords.length >= 2 && totSouthCoords.length >= 2) {
            ...
            sunsetArc = (dEndDeg > 0.02) ? buildTerminatorArc(nLast, sLast, t => computeEnvelopeLimitPoint(t, true), t => computeEnvelopeLimitPoint(t, false), 8) : [];
            sunriseArc = (dStartDeg > 0.02) ? buildTerminatorArc(sFirst, nFirst, t => computeEnvelopeLimitPoint(t, false), t => computeEnvelopeLimitPoint(t, true), 8) : [];
        }
    }
    ...
    // 4. Lóbulos del terminador y curva de Espenak
    ...
    let prevSrInside = false, prevSsInside = false;
    ...
    for (let i = 0; i <= numSteps; i++) {
        ...
        if (inside && R > 0.001) {
            ...
            if (p3) { p3.t = t; p3.x = x3; p3.y = y3; }
            if (p4) { p4.t = t; p4.x = x4; p4.y = y4; }
    ...
    // Sellado del ápice común (R = 1 + L1) para cerrar la brecha en cada lóbulo
    ...
    // fullEspenakLoop contiene ambos arcos para asegurar que ambos lóbulos tengan su línea celeste
    if (espenakSunrisePart.length > 0 && espenakSunsetPart.length > 0) {
    ...
    const allDiurnalBoundaryPts = [...fullEspenakLoop, ...espenakSunsetPart, ...sunriseLoop, ...sunsetLoop];
    ...
    // 6. Líneas de Tiempo Universal (Horas UT) — Sin distorsión en ángulo recto
    const utLines = [];
