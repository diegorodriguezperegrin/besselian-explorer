// State Management
let currentPage = 1;
const totalPages = 29; // Increased to 29 to include separate general and detailed eclipse slides
...
const pageScreenIds = [
    ...
    "screen-22",
    "screen-23",
    "screen-eclipse-general",
    "screen-eclipse-detail",
    "screen-24",
    "screen-25",
    "screen-eclipse-quiz",
    "screen-26",
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    setupEclipseGeneralAnimation();
    setupEclipseDetailAnimation();
...
function showPage(pageIndex) {
    ...
    // Show target screen
    const targetScreenId = pageScreenIds[pageIndex - 1];
...
// 10. Eclipse Mechanism - Combined View (Page 21)
function setupEclipseMechanism() {
    const generalEarthGroup = document.getElementById("comb-mech-general-earth-group");
    const generalMoonGroup = document.getElementById("comb-mech-general-moon-group");
    const generalMoonOrbitGroup = document.getElementById("comb-mech-general-moon-orbit-group");
    
    const detailOrbitLine = document.getElementById("comb-mech-detail-orbit-line");
    const detailMoonGroup = document.getElementById("comb-mech-detail-moon-group");
    const detailEarth = document.getElementById("comb-mech-detail-earth");
    const detailEarthEclipseShadow = document.getElementById("comb-mech-detail-earth-eclipse-shadow");
    const infoPanel = document.getElementById("comb-eclipse-mech-info");
    
    const tiltSlider = document.getElementById("comb-moon-tilt-slider");
    const tiltLabel = document.getElementById("comb-lbl-moon-tilt");
    const btnPreset0 = document.getElementById("comb-btn-tilt-preset-0");
    const btnPreset5 = document.getElementById("comb-btn-tilt-preset-5");
    const btnPreset15 = document.getElementById("comb-btn-tilt-preset-15");
    
    if (!generalEarthGroup || !detailMoonGroup) return;
    ...
    function updateTilt(val) {
        ...
        // Update SVG orbit line visual representation (centered at Y=200)
        if (detailOrbitLine) {
    ...
    function animate() {
        // Only run animation when screen-23 is active
        const screen23 = document.getElementById("screen-23");
        if (screen23 && !screen23.classList.contains("active")) {
            requestAnimationFrame(animate);
            return;
        }
        ...
        const ey = 200 - 90 * Math.sin(angleGeneral);
        
        const rotAngleRad = Math.atan2(-90 * Math.sin(angleGeneral), 160 * Math.cos(angleGeneral));
        ...
        const my_local = -mry * Math.sin(angleMoon);
        ...
        if (isForeground) {
            ...
            if (detailEarth && parent) {
                parent.insertBefore(detailMoonGroup, detailEarth);
            }
        }
        
        const umbra = document.getElementById("comb-mech-detail-moon-umbra");
        const penumbra = document.getElementById("comb-mech-detail-moon-penumbra");
        
        if (umbra && penumbra && detailEarthEclipseShadow) {
            const dx_rel = mx - 720;
            if (dx_rel < 0) {
                umbra.setAttribute("display", "block");
                penumbra.setAttribute("display", "block");
                
                // Earth is at Y = 200, bounds are [174, 226].
                if (my >= 174 && my <= 226) {
...
// 10b. Eclipse Mechanism - General Orbit (Page 22 - Large View)
function setupEclipseGeneralAnimation() {
    ...
    const tiltSlider = document.getElementById("gen-moon-tilt-slider");
    const tiltLabel = document.getElementById("gen-lbl-moon-tilt");
    const btnPreset0 = document.getElementById("gen-btn-tilt-preset-0");
    const btnPreset5 = document.getElementById("gen-btn-tilt-preset-5");
    const btnPreset15 = document.getElementById("gen-btn-tilt-preset-15");
    
    if (!generalEarthGroup) return;
    ...
    function animate() {
        const screenGen = document.getElementById("screen-eclipse-general");
        if (screenGen && !screenGen.classList.contains("active")) {
            requestAnimationFrame(animate);
            return;
        }
        ...
        // Earth orbits Sun at center (300, 200) - Large View
        const ex = 300 + 200 * Math.cos(angleGeneral);
        const ey = 200 - 120 * Math.sin(angleGeneral);
        
        const rotAngleRad = Math.atan2(-120 * Math.sin(angleGeneral), 200 * Math.cos(angleGeneral));
        const rotAngleDeg = rotAngleRad * 180 / Math.PI;
        ...
        const mrx = 28; // Slightly larger moon orbit radius
        const mry = 10;
        const mx_local = mrx * Math.cos(angleMoon);
        const my_local = -mry * Math.sin(angleMoon);
...
// 10c. Eclipse Mechanism - Detail Orbit & Shadow (Page 23 - Large View)
function setupEclipseDetailAnimation() {
    ...
    function updateTilt(val) {
        ...
        // Update SVG orbit line visual representation (centered at Y=180 - Large View)
        if (detailOrbitLine) {
            detailOrbitLine.setAttribute("transform", `rotate(${-currentTilt * 3}, 720, 180)`);
            const ryVal = 3 + (currentTilt * 1.8);
            detailOrbitLine.setAttribute("ry", ryVal);
        }
    ...
    function animate() {
        const screenDet = document.getElementById("screen-eclipse-detail");
        if (screenDet && !screenDet.classList.contains("active")) {
            requestAnimationFrame(animate);
            return;
        }
        ...
        const mx = 720 + rotatedX;
        const my = 180 + rotatedY; // Y center is 180 - Large View
        ...
        if (umbra && penumbra && detailEarthEclipseShadow) {
            ...
            if (dx_rel < 0) {
                ...
                // Earth is at Y = 180, bounds are [154, 206].
                if (my >= 154 && my <= 206) {
                    const isTotal = Math.abs(my - 180) < 6;
...
function setupSolarSurfaceFeatures() {
    const details = {
        ...
        "feat-fulgures": {
            desc: "Les **fúlgures** o **flamarades** són alliberaments sobtats i violents d'energia a la corona solar. Són les tempestes magnètiques més violentes del Sistema Solar i poden enviar ràfegues de radiacióó (raigs X, UV) i partícules carregades cap a la Terra en pocs minuts, afectant les nostres telecomunicacions.",
            highlight: "flare"
        },
        "feat-granuls": {
...
// 12. Knowledge Quiz Game (Page 12)
function setupKnowledgeQuiz() {
    const quizQuestions = [
        {
            question: "De què està fet principalment el Sol?",
            options: [
                "De gasos molt calents, com l'hidrogen i l'heli (plasma).",
                "De roca, lava calenta i aigua líquida com la Terra."
            ],
            correctIndex: 0,
            feedback: "Molt bé! El Sol és una estrella de gas (plasma) composta per un 74% d'hidrogen i un 24% d'heli. No té cap superfície sòlida!"
        },
        {
            question: "Quina temperatura fa al nucli (el centre) del Sol?",
            options: [
                "Uns 5.500 °C.",
                "Uns 15 milions de °C!"
            ],
            correctIndex: 1,
            feedback: "Exacte! Mentre que a la superfície (fotosfera) fa uns 5.500 °C, al centre del Sol la calor és extrema, arribant a 15 milions de graus on es produeix la fusió nuclear."
        },
        {
            question: "El Sol gira tot sencer a la mateixa velocitat?",
            options: [
                "No, l'equador gira més ràpid (25 dies) que els pols (més de 30 dies).",
                "Sí, en ser una bola rodona gira tota a la vegada."
            ],
            correctIndex: 0,
            feedback: "Correcte! Com que el Sol és fluid (plasma) i no és sòlid, experimenta rotació diferencial: el seu equador fa una volta més ràpid que les zones polars."
        }
    ];

    let currentQuestionIndex = 0;
    let kQuizScore = 0;
    const questionText = document.getElementById("k-quiz-question-text");
    const optionsContainer = document.getElementById("k-quiz-options-container");
    const feedbackText = document.getElementById("k-quiz-feedback-text");

    function renderQuestion() {
        if (!questionText || !optionsContainer || !feedbackText) return;
        feedbackText.innerHTML = "";
    ...
    function showResults() {
        const box = document.getElementById("k-quiz-box");
        if (!box) return;
        box.innerHTML = `
            <div style="text-align: center; padding: 1rem;">
                <h3 style="color: var(--solar-yellow); font-size: 1.4rem; margin-bottom: 1rem;">🏅 Joc Completat!</h3>
                <p style="font-size: 1.1rem; line-height: 1.6; margin-bottom: 1.5rem;">
                    Hem encertat <strong>${kQuizScore}</strong> de <strong>3</strong> preguntes sobre les característiques del Sol.
                </p>
                <button class="btn btn-primary" id="k-quiz-restart" style="margin: 0 auto; display: block; background: linear-gradient(135deg, var(--solar-orange), var(--solar-red)); border: none; padding: 0.6rem 1.5rem; border-radius: 8px; color: #fff; font-weight: bold; cursor: pointer;">
                    🔄 Repetir el Joc
                </button>
            </div>
        `;
        document.getElementById("k-quiz-restart").addEventListener("click", () => {
            box.innerHTML = `
                <div id="k-quiz-question-text" style="font-size: 1.2rem; font-family: var(--font-heading); font-weight: 700; color: var(--solar-yellow); margin-bottom: 1.5rem;">1. Carregant pregunta...</div>
                <div id="k-quiz-options-container" style="display: flex; flex-direction: column; gap: 1rem; margin-bottom: 1.5rem;"></div>
                <div id="k-quiz-feedback-text" style="font-size: 0.95rem; line-height: 1.6; min-height: 50px; color: var(--text-secondary); background: rgba(255,255,255,0.02); padding: 1rem; border-radius: 8px; border-left: 3px solid var(--solar-orange);"></div>
            `;
            setupKnowledgeQuiz();
        });
    }
...
// 14. Eclipse Quiz Game (Page 26)
function setupEclipseQuiz() {
    const quizQuestions = [
        {
            question: "Com es produeix un eclipsi de Sol?",
            options: [
                "Quan la Lluna passa exactament entre la Terra i el Sol, tapant la seva llum.",
                "Quan el Sol passa per darrere de la Lluna i la Lluna es torna de color vermell."
            ],
            correctIndex: 0,
            feedback: "Molt bé! Un eclipsi solar passa quan la Lluna se situa entre el Sol i la Terra, projectant la seva ombra sobre la superfície terrestre."
        },
        {
            question: "Quin tipus d'eclipsi és aquell on es veu un 'anell de foc'?",
            options: [
                "Un eclipsi anular de Sol.",
                "Un eclipsi total de Sol."
            ],
            correctIndex: 0,
            feedback: "Exacte! En un eclipsi anular, la Lluna està una mica més lluny de la Terra i es veu més petita, de manera que no tapa completament el Sol i deixa visible un espectacular 'anell de foc'."
        },
        {
            question: "Quina part del Sol només es pot veure a simple vista durant la totalitat d'un eclipsi total de Sol?",
            options: [
                "La corona solar (l'atmosfera externa del Sol).",
                "Les taques solars de la fotosfera."
            ],
            correctIndex: 0,
            feedback: "Correcte! La corona solar, que és molt tènue i només es veu com una corona de llum blanca brillant, queda al descobert quan la Lluna tapa completament el disc brillant del Sol."
        }
    ];

    let currentQuestionIndex = 0;
    let eQuizScore = 0;
    const questionText = document.getElementById("e-quiz-question-text");
    const optionsContainer = document.getElementById("e-quiz-options-container");
    const feedbackText = document.getElementById("e-quiz-feedback-text");
    ...
    function showResults() {
        const box = document.getElementById("e-quiz-box");
        if (!box) return;
        box.innerHTML = `
            <div style="text-align: center; padding: 1rem;">
                <h3 style="color: var(--solar-yellow); font-size: 1.4rem; margin-bottom: 1rem;">🏅 Joc Completat!</h3>
                <p style="font-size: 1.1rem; line-height: 1.6; margin-bottom: 1.5rem;">
                    Hem encertat <strong>${eQuizScore}</strong> de <strong>3</strong> preguntes sobre els eclipses.
                </p>
                <button class="btn btn-primary" id="e-quiz-restart" style="margin: 0 auto; display: block; background: linear-gradient(135deg, var(--solar-orange), var(--solar-red)); border: none; padding: 0.6rem 1.5rem; border-radius: 8px; color: #fff; font-weight: bold; cursor: pointer;">
                    🔄 Repetir el Joc
                </button>
            </div>
        `;
        document.getElementById("e-quiz-restart").addEventListener("click", () => {
            box.innerHTML = `
                <div id="e-quiz-question-text" style="font-size: 1.2rem; font-family: var(--font-heading); font-weight: 700; color: var(--solar-yellow); margin-bottom: 1.5rem;">1. Carregant pregunta...</div>
                <div id="e-quiz-options-container" style="display: flex; flex-direction: column; gap: 1rem; margin-bottom: 1.5rem;"></div>
                <div id="e-quiz-feedback-text" style="font-size: 0.95rem; line-height: 1.6; min-height: 50px; color: var(--text-secondary); background: rgba(255,255,255,0.02); padding: 1rem; border-radius: 8px; border-left: 3px solid var(--solar-orange);"></div>
            `;
            setupEclipseQuiz();
        });
    }
